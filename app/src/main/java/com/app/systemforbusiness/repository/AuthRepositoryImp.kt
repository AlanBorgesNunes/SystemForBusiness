...
import com.app.systemforbusiness.utils.SharedPrefConstants
...
class AuthRepositoryImp(
    ...
    override fun registerUser(
        email: String,
        password: String,
        user: Usuario, result: (UiState<String>) -> Unit
    ) {
    ...
    override fun firebaseLoginGoogle(
        idToken: String,
        email: String,
    ...
    override fun firebaseAuthGoogle(
        idToken:String,
        email:String,
        nome:String,
        user: Usuario,
    ...
    override fun updateUserInfo(user: Usuario, result: (UiState<String>) -> Unit) {
        val document = database.collection("users").document(FirebaseAuth.getInstance().currentUser?.uid!!)
        document
            .set(user)
    ...
    override fun updateUser(hashMap: Map<String,Any>, result: (UiState<String>) -> Unit) {
        val document = database.collection("users").document(FirebaseAuth.getInstance().uid!!)
        document
            .update(hashMap)
            .addOnSuccessListener {
                result.invoke(
    ...
    override fun storeSession(id: String, result: (Usuario?) -> Unit) {
        database.collection("users").document(id)
            ...
            .addOnCompleteListener {
                if (it.isSuccessful){
                    val user = it.result.toObject(Usuario::class.java)
    ...
    override fun getSession(result: (Usuario?) -> Unit) {
        val user_str = appPreferences.getString(SharedPrefConstants.USER_SESSION,null)
        if (user_str == null){
            result.invoke(null)
        }else{
            val user = gson.fromJson(user_str, Usuario::class.java)
            result.invoke(user)
        }
    }
